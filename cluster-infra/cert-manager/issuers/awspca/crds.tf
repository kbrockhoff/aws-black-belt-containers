resource "kubectl_manifest" "awspca_issuers" {
  count = var.enabled && var.install_crds ? 1 : 0
...
resource "kubectl_manifest" "awspca_cluster_issuers" {
  count = var.enabled && var.install_crds ? 1 : 0
...
spec:
  ...
  - name: v1beta1
    schema:
      openAPIV3Schema:
        description: AWSPCAClusterIssuer is the Schema for the awspcaclusterissuers
          API
        properties:
          apiVersion:
            description: 'APIVersion defines the versioned schema of this representation
